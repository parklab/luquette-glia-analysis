...
if ('snakemake' %in% ls()) {
    ...
    commandArgs <- function(...) unlist(c(
        snakemake@input['oligo_pta'],
        snakemake@input['mda_gfap'],
        snakemake@input['mda_sox10'],
...
if (length(args) != 8) {
    ...
    stop("usage: barplot_enrich_new.R oligo_pta_snv.enrich.summary.rda mda_gfap_snv.enrich.summary.rda mda_sox10_snv.enrich.summary.rda out.svg out.pdf out.csv signal_to_plot features")
}
...
outpdf <- args[5]
outcsv <- args[6]
signal_to_plot_string <- args[7]
features_string <- args[8]
...
if (!('list' %in% class(es))) {
    ...
    eslist <- list(
        `PTA Oligo SNVs`=pta[[signal.to.keep]],
        `MDA GFAP`=gfap[[signal.to.keep]],
        `MDA SOX10+/NeuN-`=sox10[[signal.to.keep]])
}
...
figwidth <- 1*((4 + 1)*em + length(rownames(x))*em*2*1.5)  # in inches
...
plot.one <- function(point.ests, bars, ylim, cols=c('red', 'purple', 'orange'), ...) { #1:length(point.ests), ...) {
...
for (i in 1:2) {
    ...
    ylim <- range(c(get.ylim(es=eslist[[1]], shift=1, scale=100, use.boot=T),
                    ...
                    get.ylim(es=eslist[[3]], shift=1, scale=100, use.boot=T)))
    plot.one(point.ests=list(setNames(eslist[[1]]$enr, rownames(eslist[[1]])), eslist[[2]]$enr, eslist[[3]]$enr),
        bars=lapply(eslist[1:3], function(es) es[, c("enr.boot.0.95.lb", "enr.boot.0.95.ub")]),
