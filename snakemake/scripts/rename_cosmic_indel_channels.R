...
if (length(args) != 2) {
    stop("usage: rename_cosmic_indel_channels.R in_cosmic.csv out.csv")
}
...
if (colnames(cosmic)[1] == 'Mutation Type') {
    # The COSMIC signatures downloaded from the website have the following ID83 channel
    # names.  The column is also called "Mutation Type", with a space.
    ...
    # The active COSMIC signatures output by SigProfilerExtractor use the same channel
    # name scheme as SCAN2 (because SCAN2 uses SigProfilerMatrixGenerator internally).
    # Just rename the column to match expectation.
    colnames(cosmic)[1] <- 'MutType'
}
