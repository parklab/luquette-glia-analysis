...
if (length(args) != 7) {
    stop('usage: fig3_panel_c2.R neuron_snvs.csv neuron_indels.csv oligo_snvs.csv oligo_indels.csv out.pdf out.svg out.csv')
}
...
n <- fread(neuron.snv)[BINSIZE==1000 & quantile %in% 1:10][order(as.integer(quantile))]
n$quantile <- 10 - as.integer(n$quantile) + 1
ni <- fread(neuron.indel)[BINSIZE==1000 & quantile %in% 1:10][order(as.integer(quantile))]
ni$quantile <- 10 - as.integer(ni$quantile) + 1

g <- fread(oligo.snv)[BINSIZE==1000 & quantile %in% 1:10][order(as.integer(quantile))]
g$quantile <- 10 - as.integer(g$quantile) + 1
gi <- fread(oligo.indel)[BINSIZE==1000 & quantile %in% 1:10][order(as.integer(quantile))]
...
for (i in 1:2) {
    ...
    par(mar=c(4,4,2,1))
    plotfn(n, g, linetype='average', xlab='Replication timing decile\nRepliSeq, binsize=1 kB', ylab='Obs/exp', main='Average SNV passAB', family='Arial')
    plotfn(n, g, labtype='point', xlab='Replication timing decile\nRepliSeq, binsize=1 kB', ylab='Obs/exp', main='Separate brain ENCODE tissues (n=15) SNV passAB', family='Arial')

    plotfn(ni, gi, linetype='average', xlab='Replication timing decile\nRepliSeq, binsize=1 kB', ylab='Obs/exp', main='Average Indel passAB', family='Arial')
    plotfn(ni, gi, labtype='point', xlab='Replication timing decile\nRepliSeq, binsize=1 kB', ylab='Obs/exp', main='Separate ENCODE tissues (n=15) Indel passAB', family='Arial')
