...
if (length(args) != 5)
    stop('usage: gene_odds_ratio.R tumor.gene_mutations.txt neuron.gene_mutations.txt oligo.gene_mutations.txt genes.bed out.rda')
...
print(genes.info)
  ...
  # XXX: old way - did not account for gene length
  #tmp <- names(counts[order(counts,decreasing=T)])
  #return(tmp)
})[[1]]
