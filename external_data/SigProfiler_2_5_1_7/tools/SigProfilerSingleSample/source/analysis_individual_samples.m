function analysis_individual_samples(signaturesSet, signaturesInSamples, ...
                                     samplesForAnalysis, outputFolder, outputFile, ...
                                     useRules, allowSigsEverywhere, connected)
 ...
 %% Analysis of data
    ...
    %% Output results 
    outputFolderText = [outputFolder filesep 'text'];
    if ( exist(outputFolder,'dir') == 0 )
       mkdir(outputFolder);
    end

    if ( exist(outputFolderText,'dir') == 0 )
       mkdir(outputFolderText);
    end
    
    outputFileTextSignatures = [outputFolderText filesep 'res_' outputFile '_signature_patterns.csv'];
    outputFileTextActivities = [outputFolderText filesep 'res_' outputFile '_signature_activities.csv'];
    
    exportSignaturesPatterns(allGenomeSignatures, sigNames, input.types, input.subtypes, ...
                             outputFileTextSignatures, ',');
                        
    exportExposures(exposuresNew, sigNames, input.sampleNames, ...
